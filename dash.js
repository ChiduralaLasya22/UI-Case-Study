...
function Dashboard(){
  ...
  const [highestMarksStudent, setHighestMarksStudent] = useState(null);
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:8000/data')
      ...
      .then((data) => {
        ...
        const highestMarks = data.reduce((max, student) =>
          student.marks > max ? student.marks : max, 0);
        const highestMarksStudent = data.find((student) => student.marks === highestMarks);
        setHighestMarksStudent(highestMarksStudent);
      })
      .catch((error) => {
      ...
      <table class="table table-striped table-dark" style={{marginTop:'15px'}}>
        ...
        <tbody>
          {students.map((student) => (
            <tr
              key={student.id}
              className={highestMarksStudent && student.id === highestMarksStudent.id ? 'highest-marks' : ''}
            >
              <td>{student.name}</td>
